...
# ╔═╡ 311a75c0-02c3-11ee-199a-c1d1d24090d9
using Images, LinearAlgebra, StaticArrays

# ╔═╡ 8e0257a0-b545-44f1-b040-b35a3be1ab95
begin
	using PlutoUI
	TableOfContents(title="Ray Tracing", indent=true)
end

# ╔═╡ 71d1b9fe-6125-401d-853c-f64728892952
begin
	import Plots, PlotlyJS
	Plots.plotlyjs()
end

# ╔═╡ 5093fa8a-fecf-4453-8135-90707f7bdc3b
html"""<style>
main {
    max-width: 1000px;
}
...
# ╔═╡ 9624daa2-7c31-4e82-b0b9-39e0b80f8275
md"# Common"

# ╔═╡ 77b25d48-2418-4112-aa41-e8a2ce19d965
md"## Data Types"

# ╔═╡ 644b4176-5051-406f-861d-301160e6bc6b
const Vec3 = SVector{3, Float64}

# ╔═╡ a58d8323-2151-4249-a090-bda32591a7d5
const Vec2 = SVector{2, Float64}

# ╔═╡ 3837c8de-244f-486c-83e5-c6547a88e39e
const Pixel = RGB{Float64}

# ╔═╡ e9c3a008-f8ee-42a6-b0be-ffcbe50ee161
const MaterialID = UInt8

# ╔═╡ 3ef57aff-499c-4b4b-b1f3-8e3e172d89e9
md"## Random"

# ╔═╡ feb809cc-1dc4-480b-993a-cabeb963a3e2
md"""
This is used for Lambertian Reflection. Using Gaussian distribution for all three coordinates of the vector will ensure an uniform distribution on the surface of the sphere.
"""

# ╔═╡ 4420c6d6-546b-4f1c-a177-711e2a0f9a14
function random_unit_vector()::Vec3 
	normalize(Vec3(randn(), randn(), randn()))
end

# ╔═╡ d807cccf-ec2d-490a-b9e8-9c7c63e4395c
Plots.scatter([Tuple(random_unit_vector()) for _ in 1:10000], markersize=0.2)

# ╔═╡ 0a6c624d-cebd-42c4-b7db-cebc74760c95
unzip(a) = map(x->getfield.(a, x), fieldnames(eltype(a)))

# ╔═╡ 0f54b482-de05-44f1-a597-dbe1affceb59
unzip([random_unit_vector() for _ in 1:100])

# ╔═╡ 05c01c80-15e9-46c6-8a81-4f724e53b5d6
function random_vector_in_unit_sphere()::Vec3
	random_unit_vector() * ∛rand()
end

# ╔═╡ d7745a15-6b66-4ebd-abac-ca99332d886e
Plots.scatter([Tuple(random_vector_in_unit_sphere()) for _ in 1:10000], markersize=0.2)
...
md"""
## Snell's Law
Зако́н Сне́ллиуса (также Снелля или Снелла) описывает преломление света на границе двух прозрачных сред. Угол падения света на поверхность связан с углом преломления соотношением:
```math
η⋅sinθ=η^′ ⋅sinθ^′
```
"""
...
begin
	n = Vec2(0, 1)
	θ = deg2rad(test_angle)
	R = Vec2(sin(θ), -cos(θ))
	η = 1.0
	η′ = test_eta
	θ′ = asin((η/η′)*sin(θ))
	R′ = Vec2(sin(θ′), -cos(θ′))
	Rcalc = refract(R, n, η, η′)
...
# ╔═╡ e5756657-6f2e-4510-b7cf-c4f3bbef6d1d
...
# ╔═╡ 808221ef-944b-48c3-ac88-3b7ebd669d0f
struct HitRecord
	p::Vec3
	normal::Vec3
	t::Float64
...
# ╔═╡ 236f4994-2468-407a-8a5b-738fa4f06134
md"# Hittable"

# ╔═╡ 599e5c03-0a81-4454-bef5-7e3605da3e77
md"## Sphere"

# ╔═╡ 8dd2c026-bbca-4d32-9b45-24ac546873db
struct Sphere
	center::Vec3
	radius::Float64
	material::MaterialID
...
# ╔═╡ 117dd47c-f109-4b09-897d-48db9b13ce2e
md"""
Уравнение сферы: 
```math 
x^2+y^2+z^2=R^2
...
P \cdot P=R^2
```
Если сфера располагается не в центре, а в позиции ``C``, тогда
```math
(P-С) \cdot (P-С)=R^2
```
В нашем случае точка P задаётся формулой ``P(t) = A + tb``. Уравнение приобретает вид
```math
(A + tb-С) \cdot (A + tb-С)=R^2
```
Раскрываем скобки и переносим ``R^2`` в левую часть и получаем
```math
...
# ╔═╡ 92691486-f8bf-4092-9528-6739f54d20e2
...
# ╔═╡ aaab7bea-643d-4b7b-b735-aaf826060bf1
...
# ╔═╡ 9949ff83-c6d1-4414-a60c-493c92c2ad26
md"# Tracing"

# ╔═╡ 1830b68f-e760-4c87-86b7-599b63fcaf81
md"# Materials"

# ╔═╡ 0b2605d5-b8ad-4f02-a734-ac99b883952d
md"### Sky"

# ╔═╡ 68d82ef4-1a96-41d7-9cda-2bbdb5883cd6
function sky_color(r::Ray)::Vec3
	t = 0.5*(r.dir.y+1.0)
	return (1.0-t)*Vec3(1,1,1) + t*Vec3(0.5, 0.7, 1.0)
end

# ╔═╡ f6db8795-bbb1-404d-804e-a0880d9eb63c
md"### Lambertian"

# ╔═╡ a4982072-7dc4-4ccd-9a05-13b6f528c460
struct Lambertian
	albedo::Vec3
end

# ╔═╡ b71c354d-8a95-45b2-8b4d-292db418122c
function get_scattered_dir(material::Lambertian, r::Ray, hit::HitRecord)
	if hit.front_face
		return hit.normal + random_unit_vector()
	end
end

# ╔═╡ f7933c61-ab2d-4ce4-a088-b3d0bc70e98f
get_attenuation(material::Lambertian, r::Ray, hit::HitRecord) = material.albedo

# ╔═╡ e17796e9-4d18-4dcb-91c4-c97c0a85bc29
md"### Metal"

# ╔═╡ 4e08986c-5e8d-41c5-a23d-adc4053c93da
struct Metal
	albedo::Vec3
	fuzz::Float64
end
...
function get_scattered_dir(material::Metal, r::Ray, hit::HitRecord)
	if hit.front_face
		return reflect(r.dir, hit.normal) + material.fuzz*random_vector_in_unit_sphere()
	end
end
...
# ╔═╡ 5e0d92de-befb-476f-b73f-d77b1684ca49
...
# ╔═╡ 4077e03c-5c48-4f0a-8599-ec781cbd0445
function ray_color(r::Ray, depth::Integer)::Vec3
	...
	scattered_dir = get_scattered_dir(material, r, hit)
	if scattered_dir isa Nothing
		return Vec3(0,0,0)
	end
	
	attenuation = get_attenuation(material, r, hit)
...
# ╔═╡ 27d809a1-e87a-4f09-89ac-5b1b1845d659
@bind samples_per_pixel Slider(1:100, default=20)

# ╔═╡ 8808657d-3422-42da-9e69-3cbef7a9e17b
samples_per_pixel

# ╔═╡ 41f3d59b-1f79-478b-8daf-77e4bb959264
@bind max_depth Slider(1:50, default=10)

# ╔═╡ 17a61e7f-1412-4f9a-b5dc-1e53f9d25033
max_depth
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "cac40f088fd45ed2f77b90d2b1a94ace4a17389e"
